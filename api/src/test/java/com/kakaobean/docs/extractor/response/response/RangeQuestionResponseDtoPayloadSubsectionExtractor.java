...
public class RangeQuestionResponseDtoPayloadSubsectionExtractor implements PayloadSubsectionExtractor {
    ...
    public byte[] extractSubsection(byte[] payload, MediaType contentType) {
        ...
        try {
            ...
            RangeQuestionResponseDto result = null;
            for (SurveyResponseDto response : findResponsesDto.getResponses()) {
                for (QuestionResponseDto questionResponse : response.getQuestionResponses()) {
                    if(questionResponse.getClass() == RangeQuestionResponseDto.class){
                        result = (RangeQuestionResponseDto) questionResponse;
                        break;
                    }
                }
            }
